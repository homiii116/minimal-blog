...
You basically don't touch original files, but make new files to override the original files.
...
This time, you modify ```footer.tsx``` in components folder, so you need to create ```components``` folder first.
...
Of course, you can make any changes wherever you want, and you can even use JS file if you don't write TypesSript :)

## Conclusion
I still learn a lot about GatsbyJS, but I'm happy if this article helps.
Gatsby theme is poweful, and I would try to customize more themes or make original themes :)
